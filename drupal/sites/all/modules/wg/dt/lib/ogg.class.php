...
// Private methods
  ...
  function Read32LE(&$buffer,$pos) {
    return(ord($buffer[$pos+0])+(ord($buffer[$pos+1])<<8)+(ord($buffer[$pos+2])<<16)+(ord($buffer[$pos+3])<<24));  // Read 32 bits little endian from buffer from index $pos
  }
  ...
  function realpath($path) { // if a path to a file is absolute, add document_root
  	if(isset($_SERVER) && !empty($_SERVER['DOCUMENT_ROOT']) && (strpos($path,$_SERVER['DOCUMENT_ROOT']))) return($path);
//    if ((strpos($path,$_SERVER['DOCUMENT_ROOT'])!==false)||(strpos($path,"://")!==false)) return($path); // path already absolute or remote
    if ((strpos($path,"://")!==false)) return($path); // path already absolute or remote
  ...
  function PackComments ($stream) {
    ...
    if (isset($stream['comments'])) {
      foreach ($stream['comments'] as $key=>$comment) if (!strlen($comment)) unset($stream['comments'][$key]);
  ...
  function analyze() { // Parse headers to retrieve identification and comments infos
    ...
    if (strpos($this->Streams['oggfile'],"http://")!==false) { // If remote file
      ...
      if ($inputfile) {
        ...
        for ($i=0; $i<66; $i++) if (!feof($inputfile)) $this->Data.=@fread($inputfile, 1024);
          else break;
    ...
    for ($pos=0; ($pos=strpos($this->Data,"OggS",$pos))!==false; $pos++) { // parse OGG pages to retrieve interresting data
      ...
      if ($this->Read32LE($this->Data,$pos+18)==0) { // Page Count = 0 : we can find the stream type and read identification headers
        ...
        elseif ((substr($this->Data,$packet+1,6)=="theora") && !isset($this->Streams['theora'])) { // decode theora identification header
          ...
          if ($theora['parn']*$theora['pard']!=0) $theora['pixelaspectratio']=$theora['parn'].":".$theora['pard'];
          else $theora['pixelaspectratio']="1:1";
          $theora['cs']=$this->readBits(8,$this->Data,$bitptr);
          if ($theora['cs']==1) $theora['colorspace']="Rec. 470M";
          elseif ($theora['cs']==2) $theora['colorspace']="Rec. 470BG";
          ...
          if ($theora['pf']==0) $theora['pixelformat']="4:2:0";
          elseif ($theora['pf']==2) $theora['pixelformat']="4:2:2";
          elseif ($theora['pf']==3) $theora['pixelformat']="4:4:4";
        }
    ...
    if (isset($this->Streams['size'])&&($this->Streams['size']>0xFFFF)) {
      if (is_resource($inputfile)) { // still open: so this is local file
        ...
        if ($inputfile) {
          ...
          for ($i=0; $i<70; $i++) if (!feof($inputfile)) $endbuffer.=@fread($inputfile, 1024);
            else break;
          fclose($inputfile);
        }
      }
